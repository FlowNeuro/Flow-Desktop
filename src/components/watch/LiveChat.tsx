...
function authorNameClass(message: LiveChatMessage): string {
  if (message.isOwner) return "text-amber-400";
  if (message.isModerator) return "text-blue-400";
  if (message.isMember) return "text-emerald-400";
...
function AuthorBadges({ message }: { message: LiveChatMessage }) {
  return (
    <>
      {message.isModerator && <ShieldCheck size={11} className="shrink-0 text-blue-400" />}
      {message.isVerified && <BadgeCheck size={11} className="shrink-0 text-neutral-400" />}
...
function ChatRow({ message }: { message: LiveChatMessage }) {
  if (message.messageType === "superChat") {
    ...
    return (
      ...
      >
        <div className={`flex items-center justify-between gap-2 text-xs font-bold ${darkText ? "text-black" : "text-white"}`}>
          <span className="truncate">{message.author}</span>
          {message.superChatAmount && <span className="shrink-0">{message.superChatAmount}</span>}
        </div>
        {message.message && (
          <div className={`mt-1 break-words text-sm ${darkText ? "text-black/90" : "text-white/90"}`}>
  ...
  if (message.messageType === "membership") {
    return (
      <div className="rounded-lg border-l-2 border-emerald-500/50 bg-emerald-500/10 px-3 py-2">
        <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-400">
          <span className="truncate">{message.author}</span>
          <AuthorBadges message={message} />
        </div>
        {message.message && (
          <div className="mt-0.5 break-words text-sm text-neutral-200">
            <MessageBody segments={message.segments} message={message.message} />
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex gap-2 px-1">
      ...
      <p className="min-w-0 break-words text-sm leading-snug text-neutral-200">
        <span
...
export function LiveChat({ videoId }: LiveChatProps) {
  ...
  return (
    <div className="flex h-[min(560px,70vh)] w-full flex-col overflow-hidden rounded-2xl border border-neutral-800 bg-surface-container-low">
      <div className="flex shrink-0 items-center gap-2 border-b border-neutral-800 px-4 py-3">
        <Radio size={16} className="text-primary" />
        <h3 className="text-base font-medium text-neutral-200">{getString("live_chat_title")}</h3>
      ...
      <div className="relative min-h-0 flex-1">
        <div ref={listRef} className="hide-scrollbar h-full space-y-2.5 overflow-y-auto px-3 py-3">
          ...
          {ended && messages.length > 0 && (
            <p className="px-1 pt-2 text-xs text-neutral-500">{getString("live_chat_ended")}</p>
          )}
        </div>
        ...
        {!pinned && (
          <button
            ...
            className="absolute bottom-3 left-1/2 flex -translate-x-1/2 items-center gap-1.5 rounded-full bg-surface-container-high px-3 py-1.5 text-xs font-semibold text-neutral-200 transition-colors hover:bg-surface-container-highest"
          >
            <ArrowDown size={13} />
