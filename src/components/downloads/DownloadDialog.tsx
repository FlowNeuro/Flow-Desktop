...
function DialogFrame({
  ...
  const panelClassName = `pointer-events-auto w-full ${compact ? "max-w-lg" : "max-w-2xl"} rounded-2xl border border-neutral-800 bg-surface p-6`;
...
function DialogHeader({
  ...
  return (
    <div className="flex items-start gap-4">
      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-surface-container-high text-neutral-200">
        {icon}
      </div>
      ...
      <button
        ...
        onClick={onClose}
        className="grid h-9 w-9 place-items-center rounded-full text-neutral-400 transition-colors duration-200 ease-out hover:bg-surface-container-high hover:text-neutral-100"
      >
        <X size={18} />
      </button>
    </div>
...
function LoadingState() {
  return (
    <div className="flex min-h-40 items-center justify-center gap-3 text-sm text-neutral-400">
...
function ErrorMessage({ message }: { message: string | null }) {
  ...
  return (
    <div role="alert" className="rounded-xl border border-red-900/50 bg-red-950/30 px-4 py-3 text-sm text-red-300">
      {message}
    </div>
  );
}
...
function DownloadProgressPanel({
  ...
  return (
    <div className="mt-6 rounded-xl border border-neutral-800 bg-surface-container-low p-4">
      <div className="flex items-center gap-3">
        <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-surface-container-high text-neutral-300">
          {status === "completed" ? (
        ...
        <div className="min-w-0 flex-1">
          <div className="text-sm font-medium text-neutral-100">{statusLabel}</div>
      ...
      {progress?.error ? (
        <div className="mt-4 rounded-lg border border-neutral-800 bg-surface-container-high p-3">
          <div className="text-xs font-semibold uppercase tracking-widest text-neutral-500">
            {getString("download_failure_reason")}
          </div>
          <p className="mt-2 whitespace-pre-wrap break-words font-mono text-xs leading-5 text-red-300">
            {progress.error}
          </p>
        </div>
      ) : null}
    ...
    <DialogFrame compact={compact} onClose={onClose}>
      ...
      {state.loading ? (
        ...
        <div className="mt-6 space-y-5">
          <div className="flex items-center gap-4 rounded-xl border border-neutral-800 bg-surface-container-low p-3">
            ...
            <div className="min-w-0">
              <div className="line-clamp-2 text-sm font-medium leading-5 text-neutral-100">
                {video?.title}
              </div>
              <div className="mt-1 truncate text-sm text-neutral-400">{video?.channelName}</div>
            </div>
          </div>
          ...
          {state.formats.length > 0 ? (
            <>
              {state.containers.length > 1 ? (
                <fieldset>
                  <legend className="mb-2 text-xs font-semibold uppercase tracking-widest text-neutral-500">
              ...
              {state.selected ? (
                <div className="grid grid-cols-3 divide-x divide-neutral-800 rounded-xl border border-neutral-800 bg-surface-container-low">
                  <div className="px-4 py-3">
                    <div className="text-xs text-neutral-500">{getString("download_video_codec")}</div>
                    <div className="mt-1 font-mono text-sm text-neutral-100">{state.selected.videoCodec}</div>
                  </div>
                  <div className="px-4 py-3">
                    <div className="text-xs text-neutral-500">{getString("download_audio_codec")}</div>
                    <div className="mt-1 font-mono text-sm text-neutral-100">{state.selected.audioCodec}</div>
                  </div>
                  <div className="px-4 py-3">
                    <div className="text-xs text-neutral-500">{getString("download_frame_rate")}</div>
          ...
          {resolvedError ? (
            <p role="alert" className="text-sm text-neutral-400">{resolvedError}</p>
          ) : null}
  ...
  return (
    <DialogFrame compact={compact} onClose={onClose} morphId={morphId}>
      ...
      <div className="mt-5">
        ...
        <div className="mt-2 max-h-56 overflow-y-auto rounded-lg border border-neutral-800 bg-surface-container-low p-3">
          {logs.length > 0 ? (
            <pre className="whitespace-pre-wrap break-words font-mono text-xs leading-5 text-neutral-300">
              {logs.join("\n")}
            </pre>
          ) : (
            <p className="text-sm text-neutral-500">{getString("download_no_logs")}</p>
          )}
        </div>
      </div>
    ...
    <DialogFrame compact={compact} onClose={onClose}>
      ...
      {state.loading ? (
        ...
        <div className="mt-6 space-y-5">
          ...
          {state.stream ? (
            <div className="rounded-xl border border-neutral-800 bg-surface-container-low px-4 py-3">
              <div className="text-xs font-semibold uppercase tracking-widest text-neutral-500">
                {getString("download_audio_format")}
              </div>
              <div className="mt-2 font-mono text-sm text-neutral-100">{describeMusicStream(state.stream)}</div>
              {!compact ? (
                <p className="mt-2 text-sm text-neutral-400">{getString("download_original_container_note")}</p>
              ) : null}
            </div>
          ) : null}
      ...
      <AnimatePresence>
        {dialog ? (
          <motion.div
            ...
            onMouseDown={close}
            className="fixed inset-0 z-[115] bg-black/70"
          />
        ) : null}
      </AnimatePresence>
